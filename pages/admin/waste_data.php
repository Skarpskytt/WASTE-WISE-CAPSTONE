...
$query = "
    SELECT 
        waste.id,
        waste.waste_date,
        COALESCE(data_management.name, ingredients.ingredient_name) AS item_name,
        waste.waste_quantity AS total_waste,
        waste.waste_value,
        waste.waste_reason,
        waste.responsible_person,
        COALESCE(data_management.image, ingredients.item_image) AS image,
        waste.item_type
    FROM waste
    LEFT JOIN data_management ON waste.item_id = data_management.id AND waste.item_type = 'product'
...
if (!empty($search)) {
    $query .= " AND data_management.name LIKE :search";
...
if (!empty($search)) {
    $countQuery .= " AND data_management.name LIKE :search";
}
...
if (isset($_POST['export_pdf'])) {
    try {
        ...
        $stmt = $pdo->query("
            SELECT 
                w.id,
                COALESCE(dm.name, ing.ingredient_name) as item_name,
                w.waste_date,
                w.waste_quantity,
                w.waste_value,
                w.waste_reason,
                ...
                w.item_type
            FROM waste w
            LEFT JOIN data_management dm ON w.item_id = dm.id AND w.item_type = 'product'
