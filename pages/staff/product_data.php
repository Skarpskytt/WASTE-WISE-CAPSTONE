...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        try {
            // Update table name from inventory to data_management
            $stmt = $pdo->prepare("INSERT INTO data_management (name, category, quantity, unit, location, stock_date, image, price_per_unit) VALUES (:name, :category, :quantity, :unit, :location, :stock_date, :image, :price_per_unit)");
            $stmt->execute([
                ':name' => $itemName,
                ':category' => $category,
...
// Retrieve inventory items from database using PDO - update table name
try {
    $stmt = $pdo->query("SELECT * FROM data_management ORDER BY created_at DESC");
...
<?php include (__DIR__ . '/../../layout/staff_nav.php'); ?>
 ...
 <div class="p-7">
  ...
  <div>
    ...
    <p class="text-gray-500 mt-2">Manage your Product</p>
  </div>
