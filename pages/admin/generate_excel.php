...
$dataQuery = "SELECT pw.id, p.name as product_name, p.category, pw.waste_date, 
              pw.waste_quantity, p.quantity_produced as product_quantity_produced,
              pw.quantity_sold, pw.waste_value, pw.waste_reason, pw.disposal_method,
...
// Create Excel file
$fileName = "{$branchName}_product_waste_report_" . date('Y-m-d') . ".csv";
...
// Add headers
fputcsv($output, [
    'Report: ' . $branchName . ' Product Waste Data'
]);
...
// Add column headers
fputcsv($output, [
    'ID', 'Product', 'Category', 'Waste Date', 'Quantity', 'Produced', 
    'Sold', 'Value (₱)', 'Reason', 'Disposal Method', 'Staff'
]);
